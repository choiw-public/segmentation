...
class ModelFunction(Model):
    ...
    def call(self, tensor_in, training=False, **kwargs):
        root = self.root(tensor_in, training=self.training)

        encoder1a = self.encoder1a(root, training=self.training)
        encoder1a_skip = self.encoder1a_skip(encoder1a, training=self.training)
        encoder1b = self.encoder1b(encoder1a, training=self.training)

        encoder2a = self.encoder2a(encoder1b, training=self.training)
        encoder2a_skip = self.encoder2a_skip(encoder2a, training=self.training)
        encoder2b = self.encoder2b(encoder2a, training=self.training)

        encoder3a = self.encoder3a(encoder2b, training=self.training)
        encoder3a_skip = self.encoder3a_skip(encoder3a, training=self.training)
        encoder3b = self.encoder3b(encoder3a, training=self.training)

        encoder4a = self.encoder4a(encoder3b, training=self.training)
        encoder4b = self.encoder4b(encoder4a, training=self.training)

        decoder1 = self.decoder1(encoder4b, training=self.training)
        _, h, w, _ = get_shape(encoder3a_skip)
        decoder1 = tf.image.resize(decoder1, [h, w]) + encoder3a_skip

        decoder2 = self.decoder2(decoder1, training=self.training)
        _, h, w, _ = get_shape(encoder2a_skip)
        decoder2 = tf.image.resize(decoder2, [h, w]) + encoder2a_skip

        decoder3 = self.decoder3(decoder2, training=self.training)
